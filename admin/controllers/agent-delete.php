...
if (isset($_POST['id'])) {
    ...
    try {
        // Prepare the SQL DELETE query for agents
        $delete_query_agent = "DELETE FROM agents WHERE agent_id = ?";
        ...
        if ($stmt_agent = mysqli_prepare($conn, $delete_query_agent)) {
            // Bind the agent ID
            mysqli_stmt_bind_param($stmt_agent, "i", $agent_id);
        ...
        if ($stmt_user = mysqli_prepare($conn, $delete_query_user)) {
            ...
            mysqli_stmt_bind_param($stmt_user, "i", $agent_id);
        ...
        header("Location: agents-list?status=success-delete");
    } catch (Exception $e) {
        // Rollback the transaction in case of any error
        mysqli_rollback($conn);

        // Redirect with an error message
        header("Location: agents-list.php");
    }
} else {
    // If the agent ID is not set, redirect with an error message
    header("Location: agents-list");
}
